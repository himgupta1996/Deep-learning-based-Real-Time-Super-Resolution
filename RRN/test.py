...
os.system('rm -rf `find -type d -name .DS_Store`')
print('deleted hidden checkpoints')
...
parser.add_argument('--threads', type=int, default=1, help='number of threads for data loader to use') #32
parser.add_argument('--seed', type=int, default=123, help='random seed to use. Default=123') #0
parser.add_argument('--gpus', default=1, type=int, help='number of gpu') #1
parser.add_argument('--cuda',default=True, type=bool)
parser.add_argument('--layer', type=int, default=5, help='network layer')
#parser.add_argument('--test_dir',type=str,default='./data/Vid4')
#parser.add_argument('--test_dir',type=str,default='./reds/val/val_sharp')
parser.add_argument('--test_dir',type=str,default='./VidCat/animation/GT')
...
parser.add_argument('--pretrain', type=str, default='./result2/weight/2022-04-25-01-23/X4_5L_64_epoch_9.pth')
#./model/RRN-5L.pth
#./result/weight/2022-04-19-18-32/X4_5L_64_epoch_57.pth
#./result2/weight/2022-04-25-01-23/X4_5L_64_epoch_9.pth
parser.add_argument('--image_out', type=str, default='./out-animation-qat/')
...
def main():
    ...
    count = 0
    scene_list=['01']
    #scene_list=['realvsr15900',  'realvsr48427',  'tos3face9889',  'tos3face9968',  'tos3room4457']
    #scene_list=['building/018', 'building/028', 'car/223', 'crowd/484', 'miscellaneous/159']
    #scene_list=['002', '01', '012', '027', '484', 'walk']
    #scene_list = ['00'+str(int(i)) for i in range(10)] #reds val sharp
    #scene_list += ['0'+str(int(i)) for i in range(10,30)] #reds val sharp
    ...
    for scene_name in scene_list:
        ...
        #t_end=time.time()
        #print("===> Timer for set: %.4f sec." % (t_end - t_start))
        PSNR_avg += PSNR
        SSIM_avg += SSIM
        count += 1
...
def test(test_loader, rrn, scale, scene_name, n_c):
    ...
    for image_num, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(T):
                if init:
                    ...
                    init = False
                    ty=time.time()
                    print("time for init: %.4f sec." % (ty - tx))
                else:
                    c+=1
                    #tx=ty=time.time()
        ...
        t1 = time.time()
        print("===> Timer: %.4f sec." % (t1 - t0))
        print("===> Timer without init: %.4f sec." % (t1 - t0 - (ty - tx)))
        print("True FPS = ",c/(t1 - t0 - (ty - tx)))
        for i in range(T):
        ...
        t2=time.time()
        print("===> Timer after saving image: %.4f sec." % (t2 - t0))
        for i in range(T):
            #save_img(prediction[i], scene_name, i)
            # test_Y______________________
            prediction_Y = bgr2ycbcr(prediction[i])
