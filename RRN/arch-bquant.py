...
class ResidualBlock_noBN(nn.Module):
    ...
    def __init__(self, nf=64):
        ...
        self.conv1 = qnn.QuantConv2d(nf, nf, 3, 1, 1, bias=True, weight_bit_width=3)
        self.conv2 = qnn.QuantConv2d(nf, nf, 3, 1, 1, bias=True, weight_bit_width=3)
...
class neuro(nn.Module):
    def __init__(self, n_c, n_b, scale):
        ...
        pad = (1,1)
        block = []
        self.conv_1 = qnn.QuantConv2d(scale**2*3 + n_c + 3*2, n_c, (3,3), stride=(1,1), padding=pad, weight_bit_width=3)
        basic_block = functools.partial(ResidualBlock_noBN, nf=n_c)
        self.recon_trunk = make_layer(basic_block, n_b)
        self.conv_h = qnn.QuantConv2d(n_c, n_c, (3,3), stride=(1,1), padding=pad, weight_bit_width=3)
        self.conv_o = qnn.QuantConv2d(n_c, scale**2*3, (3,3), stride=(1,1), padding=pad, weight_bit_width=3)
