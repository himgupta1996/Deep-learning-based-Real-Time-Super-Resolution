...
class RRN(nn.Module):
    ...
    def forward(self, x, x_h, x_o):
        ...
        x_o = self.down(x_o)
        x_h, x_o = self.neuro(x_input, x_h, x_o)
        x_o = self.skip_add.add(F.pixel_shuffle(x_o, self.scale), F.interpolate(f2, scale_factor=self.scale, mode='bilinear', align_corners=False))
        return x_h, x_o
