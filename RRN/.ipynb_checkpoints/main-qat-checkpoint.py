...
parser.add_argument('--nEpochs', type=int, default=70, help='number of epochs to train for')#70
parser.add_argument('--snapshots', type=int, default=3, help='Snapshots. This is a savepoint, using to save training model.')
parser.add_argument('--lr', type=float, default=0.0001, help='Learning Rate. Default=0.01')#0.0001
...
parser.add_argument('--layer', type=int, default=5, help='network layer')#10
...
parser.add_argument('--save_model_path', type=str, default='./result2/weight/', help='Location to save checkpoint models')
parser.add_argument('--save_train_log', type=str ,default='./result2/log/')
parser.add_argument('--weight-decay', default=5e-04, type=float,help="weight decay (default: 5e-04)")
parser.add_argument('--log_name', type=str, default='rrn-5')#rrn-10
parser.add_argument('--other_dataset', type=bool, default=False, help="If True using vid4k(test),else using vimo90k")
parser.add_argument('--gpu-devices', default='0', type=str, help='gpu device ids for CUDA_VISIBLE_DEVICES') #'0,1,2,3'
parser.add_argument('--checkpoint_path',default='',type=str,help='checkpoint to resume training') # ''
...
def main():
    ...
    #print()
    #print('**rnn before load',rrn)
    #print()
    #print('**optimizer before load',optimizer)
    #print()
    #print('**criterion before load',criterion)
    #print()
    if len(opt.checkpoint_path)>0: #quant
        print("=> loading checkpoint '{}'".format(opt.checkpoint_path))
        ckpt = torch.load(opt.checkpoint_path)
        start_epoch = ckpt['epoch']
        rrn.load_state_dict(ckpt['state_dict'])
        #print('**rnn after load',rrn)
        #print()
        optimizer.load_state_dict(ckpt['optimizer'])
        #print('**optimizer after load',optimizer)
        #print()
        criterion = ckpt['loss']
        #print('**criterion after load',criterion)
        #print()
    else:
        start_epoch=0

    for epoch in range(max(int(start_epoch)+1,int(opt.start_epoch)), opt.nEpochs+1): #quant
        train(train_loader, rrn, opt.scale, criterion, optimizer, epoch, use_gpu, n_c) #fed data into network
        scheduler.step()
        if (epoch) % (opt.snapshots) == 0:
            checkpoint(rrn, epoch, optimizer, criterion)

def train(train_loader, rrn, scale, criterion, optimizer, epoch, use_gpu, n_c):
    train_mode = True
    epoch_loss = 0
    rrn.train()
    rrn.qconfig = torch.quantization.get_default_qat_qconfig('fbgemm') #quant
    #rrn = torch.quantization.fuse_modules(rrn,[['conv1','relu'], ['submodule.conv', 'submodule.relu']]) #quant
    #rrn = torch.quantization.fuse_modules(rrn,[['conv_h','relu'], ['submodule.conv', 'submodule.relu']])
    rrn = torch.quantization.prepare_qat(rrn) #quant
...
def checkpoint(rrn, epoch, optimizer, criterion): 
    #print('**1 checkpoint')
    rrn = torch.quantization.convert(rrn) #quant
    ...
    #state = {
    #'epoch': epoch,
    #'state_dict': rrn.state_dict(),
    #'optimizer': optimizer.state_dict(),
    #'loss': criterion,
    #}
    #print('**4 checkpoint')
    #model_name= 'X'+str(opt.scale)+'_{}L'.format(opt.layer)+'_{}'.format(opt.patch_size)+'_epoch_{}_for_train.pth'.format(epoch)
