...
class ResidualBlock_noBN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        x=self.dequant1(x)
        identity=self.dequant1(identity)
        return self.f_add.add(identity,out) #identity + out
...
class neuro(nn.Module):
    ...
    def forward(self, x, h, o):
        ...
        x_h = self.relu(x_h)
        x_o = self.conv_o(x)
        x_h, x_o=self.dequant2(x_h), self.dequant2(x_o)
        return x_h, x_o
...
class RRN(nn.Module):
    def __init__(self, scale, n_c, n_b):
        ...
        self.n_c = n_c
        self.f2_add=nn.quantized.FloatFunctional()
    ...
    def forward(self, x, x_h, x_o, init):
        ...
        if init:
            ...
            x_o = self.down(x_o)
            x_h, x_o = self.neuro(x_input, x_h, x_o)
        x_o = self.f2_add.add(F.pixel_shuffle(x_o, self.scale) , F.interpolate(f2, scale_factor=self.scale, mode='bilinear', align_corners=False))
