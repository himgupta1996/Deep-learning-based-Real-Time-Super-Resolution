...
def main():
    ...
    p = sum(p.numel() for p in old_rrn.parameters())*4/1048576.0
    print('Model Size: {:.2f}M'.format(p))
    print('===> {}L model has been initialized'.format(n_b))
    old_rrn = torch.nn.DataParallel(old_rrn)
    criterion = nn.L1Loss(reduction='sum')
    if use_gpu:
        old_rrn = old_rrn.cuda()
        criterion = criterion.cuda()
    optimizer = optim.Adam(old_rrn.parameters(), lr = opt.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=opt.weight_decay) 
    if opt.stepsize > 0:
        scheduler = lr_scheduler.StepLR(optimizer, step_size = opt.stepsize, gamma=opt.gamma)
    qqrrn = QuantizedRNN(old_rrn)
    ...
    for iteration, data in enumerate(train_loader):
        ...
        for i in range(T-1):
            if init:
                ...
                h, prediction = qrrn(x_input[:,:,i:i+2,:,:], init_h, init_o)
                out.append(prediction)
                init = False
            else:
                h, prediction = qrrn(x_input[:,:,i:i+2,:,:], h, prediction)
                out.append(prediction)
