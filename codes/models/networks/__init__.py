...
def define_generator(opt):
    ...
    elif net_G_opt['name'].lower() == 'egvsr':
        # efficient GAN-based generator
        print("I am here before importing FRNet")
        from .egvsr_nets import FRNet
        print("I am here after importing FRNet")
        net_G = FRNet(
            in_nc=net_G_opt['in_nc'],
            out_nc=net_G_opt['out_nc'],
            nf=net_G_opt['nf'],
            ...
            degradation=opt['dataset']['degradation']['type'],
            scale=opt['scale'])
        print("I am after initializing FRNet")
